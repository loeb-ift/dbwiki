...
本系統提供一個由 AI 驅動的自動化分析流程，旨在從您的資料庫中識別、分析並範本化唯一識別碼（如訂單號、產品編號等）。此功能的核心邏輯由 `serial_number_candidate_generation` 和 `pattern_and_template_generation` 兩個提示詞驅動。
...
1.  **階段一：候選欄位識別 (由 LLM 執行)**
    *   **目標**：從資料庫中找出所有可能是流水號或唯一識別碼的欄位。
    *   **作法**：
        1.  系統會整合您上傳的所有 **DDL 結構**、**業務文件**和 **SQL 問答範例**作為上下文。
        2.  接著，呼叫 `serial_number_candidate_generation` 提示詞，要求 LLM 根據這些上下文，推斷出哪些欄位可能是唯一識別碼。
        3.  LLM 會回傳一個 JSON 列表，其中包含候選的 `table_name`、`column_name` 以及它判斷的理由。

2.  **階段二：資料特徵提取 (由 Python 腳本執行)**
    *   **目標**：對第一階段找出的每個候選欄位進行量化分析，以收集更多證據。
    *   **作法**：
        1.  系統會遍歷所有 SQL 問答範例。
        2.  如果某個 SQL 的 `WHERE` 條件中使用了候選欄位進行精確匹配（如 `WHERE order_no = 'ABC-123'`），系統會提取出 `'ABC-123'` 這樣的值作為樣本。
        3.  收集到足夠的樣本值後，Python 腳本會計算這些值的統計特徵，例如：
            *   長度分佈 (最小、最大、平均長度)
            *   唯一性比例
            *   字元類型構成 (字母、數字、特殊符號的比例)

3.  **階段三 & 四：模式識別與模板生成 (由 LLM 執行)**
    *   **目標**：從提取出的特徵和樣本中，歸納出通用的規則和模板。
    *   **作法**：
        1.  系統將前兩個階段的結果（候選欄位、LLM 的理由、樣本值、統計特徵）打包成一個詳細的 JSON 物件。
        2.  呼叫 `pattern_and_template_generation` 提示詞，將這個 JSON 物件傳給 LLM，要求它完成更高層次的分析。
        3.  LLM 會回傳一份非常詳細的綜合報告，內容包括：
            *   **模式分析**：將相似的流水號格式進行聚類（例如，'OR-' 開頭的歸為一類，'PRD-' 開頭的歸為另一類）。
            *   **結構拆解**：分析流水號的組成結構，如 `[前綴]-[日期]-[序號]`。
            *   **模板生成**：為每種模式生成一個包含正則表達式 (Regex)、元件說明、驗證建議的標準模板。
            *   **異常報告**：列出不符合任何主要模式的異常值。
            *   **最終建議**：提供關於資料庫約束（例如，是否應加上 `UNIQUE` 索引）和資料品質的改善建議。

這個重構後的流程將原本單一、模糊的分析任務，拆解成幾個目標明確、可驗證的步驟，大幅提高了分析的準確性和實用性。
