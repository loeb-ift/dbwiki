...
-- 這個查詢用於：[請在此描述查詢的業務目的]
-- 頻率：1
command-a-03-2025"  # Cohere's default model
...
-- 這個查詢用於：[請在此描述查詢的業務目的]
-- 頻率：1
sql') or (type eq 'ddl') or (type eq 'doc')"
        ).by_page()
...
-- 這個查詢用於：[請在此描述查詢的業務目的]
-- 頻率：1
ddl_index: ", ddl_index, " question_sql_index: ",
          question_sql_index)
...
-- 這個查詢用於：[請在此描述查詢的業務目的]
-- 頻率：1
]['ddl'] for hit in response['hits']['hits']]
...
-- 這個查詢用於：[請在此描述查詢的業務目的]
-- 頻率：1
n_results_sql", config.get("n_results", 10))
        self.n_results_documentation = config.get("n_results_documentation", config.get("n_results", 10))
        self.n_results_ddl = config.get("n_results_ddl", config.get("n_results", 10))
...
-- 這個查詢用於：[請在此描述查詢的業務目的]
-- 頻率：1
: "sql"})[["question", "sql"]].to_dict(orient="records")
...
-- 這個查詢用於：[請在此描述查詢的業務目的]
-- 頻率：1
DELETE FROM `{self.table_id}` WHERE id = '{id}';
