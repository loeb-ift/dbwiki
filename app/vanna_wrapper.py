...
class MyVanna(BaseMyVanna):
    ...
    def generate_sql(self, question, **kwargs):
        try:
            logger.info(f"Starting SQL generation for question: {question[:100]}...")
            logger.debug(f"Generate SQL kwargs before processing: {kwargs}")
            ...
            logger.error(f"An unexpected error occurred in generate_sql: {e}")
            logger.error(f"Stack trace: {traceback.format_exc()}")
            raise
    ...
    def run_sql(self, sql: str):
        ...
        try:
            ...
            # Instead of re-raising, return an empty DataFrame to prevent crashes
